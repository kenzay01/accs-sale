...
class DatabaseManager {
  ...
  init() {
    ...
    this.db.serialize(() => {
      ...
      // Data migration: fix legacy orders where user_id stored telegram_id
      // Map orders.user_id from users.telegram_id to users.id when applicable
      this.db.run(
        `UPDATE orders
         SET user_id = (
           SELECT u.id FROM users u WHERE u.telegram_id = orders.user_id
         )
         WHERE EXISTS (
           SELECT 1 FROM users u WHERE u.telegram_id = orders.user_id
         )`
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      this.getUser(userTelegramId).then(user => {
        ...
        `, [user.id, productName, price], function(err) {
          if (err) reject(err);
          else resolve(this.lastInsertRowid);
        });
      }).catch(reject);
    });
  }
