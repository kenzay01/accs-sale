...
export default function CategorySelector({
  ...
  return (
    <div className="p-4">
      {!selectedCategory && (
        <h2 className="text-lg font-semibold mb-2">Головна</h2>
      )}
      ...
      {!selectedCategory ? (
        <div className="flex gap-3 flex-nowrap overflow-x-auto pt-6 pb-4">
          {categories.map((category) => {
            const label =
              currentLanguage === "ru" ? category.label_ru : category.label_en;
            return (
              <CategoryItem
                key={category.id}
                id={category.id}
